...
public class UserServiceImpl implements UserService {
    ...
    return PaginationUtils.toPagePageResponse(userPage,UserResponse::fromEntity);
}
    @Override
    ...
    User user=serviceUtils.getUserByIdOrThrow(id);
    if (user.getEmail().equalsIgnoreCase(currentUserEmail)){
        throw new RuntimeException("You cannot deactivate your own status.");
    }
    ensureNotLastActiveAdmin(user);
    ...
    public MessageResponse changeUserRole(Long id, UserRequest userRequest) {
       ...
       if (user.getRole()==Role.ADMIN && newRole!=Role.USER){
        ensureNotLastActiveAdmin(user,"change role of");
       }
